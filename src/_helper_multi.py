...
class DataLoader:
    ...
    def preprocessing_multi(
        ...
        if y.ndim == 2:
            y = y[np.newaxis, :, :]
        self.valid_steps = np.sum(np.isfinite(y).all(axis=2), axis=1).astype(int)
        ...
        for idx in range(y.shape[0]):
            ...
            val = y_sample[
                (int(train_size * valid_steps_sample) - self.horizon):
                int((train_size + val_size) * valid_steps_sample)
            ]
            test = y_sample[
        ...
        self.X_train, self.Y_train, self.train_idxs = self.create_sequences(
            train_lst, self.horizon, self.back_horizon, self.sequence_stride, n_features
        )
        self.X_val, self.Y_val, self.val_idxs = self.create_sequences(
            val_lst, self.horizon, self.back_horizon, self.sequence_stride, n_features
        )
        self.X_test, self.Y_test, self.test_idxs = self.create_sequences(
            test_lst, self.horizon, self.back_horizon, self.sequence_stride, n_features
        )
    ...
    @staticmethod
    def create_sequences(series_lst, horizon, back_horizon, sequence_stride, n_features):
        Xs, Ys, sample_idxs = [], [], []
        for idx, series in enumerate(series_lst):
            len_series = series.shape[0]
            min_len = back_horizon + horizon
            if len_series < min_len:
                pad_len = min_len - len_series
                series = np.pad(series, ((0, pad_len), (0, 0)), mode='constant', constant_values=0)
                len_series = series.shape[0]
            for i in range(0, len_series - back_horizon - horizon + 1, sequence_stride):
                x_seq = series[i : i + back_horizon]
                y_seq = series[i + back_horizon : i + back_horizon + horizon]
                if x_seq.shape[0] < back_horizon:
                    x_seq = np.pad(x_seq, ((0, back_horizon - x_seq.shape[0]), (0, 0)), mode='constant')
                if y_seq.shape[0] < horizon:
                    y_seq = np.pad(y_seq, ((0, horizon - y_seq.shape[0]), (0, 0)), mode='constant')
                Xs.append(x_seq)
                Ys.append(y_seq)
                sample_idxs.append(idx)
        return (
            np.array(Xs).astype(np.float32),
            np.array(Ys).astype(np.float32),
            np.array(sample_idxs)
        )
